 ...
 *   Row 2 = headers: Name | Email | Region | Location |
 *            New active agent recruitment | Newly active agents |
...
async function getFieldCheckinsSummary(sheets, sid, userEmail) {
  ...
  try {
    ...
    rows.forEach(row => {
      ...
      const loc = row[7] ? String(row[7]) : '';
      if (loc) locations.add(loc.substring(0, 20));

      // Column layout: F(5)=PhotoPreview  G(6)=PhotoURL  H(7)=Location
      //                J(9)=Notes  L(11)=ActivityType
      const photo        = row[5]  ? String(row[5])  : '';  // PhotoPreview — direct display URL
      const activityType = row[11] ? String(row[11]) : '';  // ActivityType
      const notes        = row[9]  ? String(row[9])  : '';  // Notes
