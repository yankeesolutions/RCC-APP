...
function buildTeamList(rows, cols, month) {
  // Each row in this sheet = one RCC, not a sub-agent.
  // Sub-agent / individual agent data is not available in this sheet.
  // Return an empty list so the Team tab shows no members (correct behaviour).
...
async function getFieldCheckinsSummary(sheets, sid, userEmail) {
  ...
  try {
    ...
    rows.forEach(row => {
      ...
      // Correct checkin.js  → photoUrl=row[8], activityType=row[11], notes=row[12]
      // Truncated checkin.js → activityType=row[8], notes=row[9] (no photo columns)
      const isDriveUrl = (s) => typeof s === 'string' && (s.startsWith('https://drive.google') || s.startsWith('https://docs.google') || s.startsWith('https://lh'));
      ...
      recent.push({ id: row[0]||'', timestamp: ts.toISOString(),
        rccEmail: row[2]||'', rccName: row[3]||'', location: loc || row[7] || '',
