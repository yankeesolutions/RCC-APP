 ...
 *   Row 2 = headers: Name | Email | Region | Location | NMV columns | Agent columns | OP columns
...
function buildProgress(userRow, cols) {
  ...
  return {
    nmv:    { actual: toNum(userRow[cols.PERSONAL_NMV]) + toNum(userRow[cols.TEAM_NMV]), target: toNum(userRow[cols.NMV_TARGET]) },
    agents: { actual: toNum(userRow[cols.NEW_AGENTS]),  target: toNum(userRow[cols.AGENT_TARGET]) },
    ops:    { actual: toNum(userRow[cols.NEW_OPS]),     target: cols.OP_TARGET >= 0 ? toNum(userRow[cols.OP_TARGET]) : 0 }
  };
}
...
async function getFieldCheckinsSummary(sheets, sid, userEmail) {
  ...
  try {
    ...
    rows.forEach(row => {
      ...
      const loc = row[7] || row[6] || '';
      if (loc) locations.add(String(loc).substring(0, 20));

      // Column compatibility: correct checkin.js writes photo=col8, activity=col11
      // Current (truncated) checkin.js writes activity=col8 with no photo
      const isDriveUrl = (s) => typeof s === 'string' &&
        (s.startsWith('https://drive.google') || s.startsWith('https://docs.google') || s.startsWith('https://lh'));
      const col8  = row[8]  != null ? String(row[8])  : '';
      const col11 = row[11] != null ? String(row[11]) : '';
      const col12 = row[12] != null ? String(row[12]) : '';
      const col9  = row[9]  != null ? String(row[9])  : '';
      const photo        = isDriveUrl(col8) ? col8 : (isDriveUrl(col11) ? col11 : '');
      const activityType = col11 && !isDriveUrl(col11) ? col11 : (!isDriveUrl(col8) ? col8 : '');
      const notes        = col12 || col9 || '';

      recent.push({ id: row[0]||'', timestamp: ts.toISOString(),
        rccEmail: row[2]||'', rccName: row[3]||'', location: loc || '',
        photo, activityType, notes });
    });
